...
void pitch2category_process(pitch2category_obj *obj, const pitches_obj *pitches, const tracks_obj *tracks, categories_obj *categories)
{
    ...
    for (iSep = 0; iSep < obj->nSeps; iSep++)
    {
        ...
        if (tracks->ids[iSep] != 0)
        {
            ...
            if (obj->categories[iSep] != 0x01 && pitches->array[iSep] != 0.0)
//            if (obj->categories[iSep] != 0x01)
            {
                ...
                // v_i
                if (fabs(obj->phisNow[iSep]) < obj->phiMin) printf("phi %f2.2\n");
...
//            if (obj->categories[iSep] != 0x01) printf("%llu %1.1f %3.1f %3.1f %2.2f\n", tracks->ids[iSep], tracks->activity[iSep], pitches->array[iSep], obj->phisNow[iSep], obj->rs[iSep]);
            if (obj->categories[iSep] != 0x01) 
                printf("%2.2f,", obj->r0 - obj->rs[iSep]);
